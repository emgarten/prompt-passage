...
def lint(c: Context) -> None:
    ...
    Format and lint
    """
    c.run("ruff check src tests tasks.py --fix")
    c.run("black src tests tasks.py")


@task
def type_check(c: Context) -> None:
    c.run("mypy src tasks.py")


@task
def test(c: Context) -> None:
    c.run("pytest tests --disable-warnings -v")


@task
def service_up(c: Context) -> None:
    c.run("uvicorn src.local_llm_proxy.proxy_app:app --reload --host 0.0.0.0 --port 8000")
